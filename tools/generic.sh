...
# usage:  tool_error <caller> <message> [<errorcode>]
tool_error() {
    caller=${1:-""}
    msg=${2:-""}
    exitcode=${3:-1}
    if [ -z "${caller}" -o -z "${msg}" ]; then
        echo "usage: tool_error <caller's name> <message> [<exitcode>]"
        exit 1
    fi
...
# Non-exiting version of tool_error in case we wish to accumulate errors and
# call an exiting tool_error after displaying accumulated errors.
tool_error_noexit() {
    caller=${1:-""}
    msg=${2:-""}
    if [ -z "${caller}" -o -z "${msg}" ]; then
...
tool_warning() {
    ...
    exitcode=${3:-1}
    if [ -z "${caller}" -o -z "${msg}" ]; then
        echo "usage: tool_warning <caller's name> <message> [<exitcode>]"
        exit 1
    fi
    caller=`basename ${caller} | tr [a-z] [A-Z]`
    echo -en '\E[40;36m'"\033[1m[${caller}]\033[0m "
...
# usage:  tool_info <caller> <message>
tool_info() {
    caller=${1:-""}
    shift
    if [ -z "${caller}" -o -z "$*" ]; then
        echo "usage: tool_info <scriptname> <msg1> [<msg2> .. <msgN>]"
        exit 1
    fi
    caller=`basename ${caller} | tr [a-z] [A-Z]`
...
# usage:
#   tool_exec <calling script> [OPTS] <command to run>
#  OPTS:
